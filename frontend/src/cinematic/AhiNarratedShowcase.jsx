...
import { industryGroups } from '../data/industryCatalog';
...
const sceneIndexByKind = { home: 0, library: 1, chat: 2, apps: 3, code: 4, browser: 5, computer: 6, teamspaces: 7 };
...
export const AhiNarratedShowcase = ({ scenarios, activeId, onScenarioSelect }) => {
  ...
  return <section ref={sectionRef} className="ahi-story ahi-industry-story is-compact" id="ahi-live" aria-labelledby="ahi-compact-title" data-testid="ahi-live-story">
    <div className="ahi-proof-column" data-testid="ahi-sticky-product">
      ...
      <div className="ahi-industry-selector" data-testid="ahi-industry-controls"><label htmlFor="ahi-industry-select"><span>INDUSTRY / ICP</span><select id="ahi-industry-select" value={scenario.id} onChange={(event) => selectIndustry(event.target.value)} data-testid="ahi-industry-select">{industryGroups.map((group) => <optgroup label={group.label} key={group.label}>{group.ids.map((id) => { const item = scenarios.find((entry) => entry.id === id); return item ? <option value={id} key={id}>{item.label}</option> : null; })}</optgroup>)}</select></label><div><span>{scenario.group || 'KNOWLEDGE WORK'}</span><strong data-testid="ahi-active-icp-role">{scenario.role}</strong></div></div>
    ...
    <div className="ahi-narrative-column" data-testid="ahi-chapter-narration">
      <span className="ahi-compact-kicker">30 / 70 PRODUCT PROOF</span><h2 id="ahi-compact-title">One interface.<br />Every knowledge worker.</h2><p className="ahi-machine-summary" data-testid="ahi-machine-summary">Eight real surfaces turn one human intent into grounded specialist work, visible approval, controlled execution, and retained organizational memory.</p>
