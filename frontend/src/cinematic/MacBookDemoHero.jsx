...
export const MacBookDemoHero = ({ scenarioId, onScenarioChange }) => {
  ...
  useEffect(() => {
    const syncMotion = () => setSiteReduced(document.documentElement.dataset.motion === 'reduced');
    window.addEventListener('acoord:motion', syncMotion);
    return () => window.removeEventListener('acoord:motion', syncMotion);
  }, []);
