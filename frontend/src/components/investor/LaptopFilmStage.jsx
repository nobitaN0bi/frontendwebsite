...
export const LaptopFilmStage = ({ film, active, onSceneChange }) => {
  ...
  return <div className="laptop-film-stage" data-testid={`${film.id}-laptop-stage`}>
    ...
    <motion.div className="laptop-film-shell" animate={{ scale: reduced ? 1 : .965 + active * .009, rotateX: reduced ? 0 : Math.max(0, 3 - active * .75), y: reduced ? 0 : active === 0 ? 12 : 0 }} transition={{ duration: reduced ? 0 : .7, ease: [0.16, 1, 0.3, 1] }}>
    ...
    <AnimatePresence mode="wait"><motion.article key={`${active}-action`} className={`laptop-action-callout callout-${current.position}`} initial={reduced ? false : { opacity: 0, y: 18, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={reduced ? undefined : { opacity: 0, y: -10 }} transition={{ duration: reduced ? 0 : .4, ease: [0.16, 1, 0.3, 1] }} data-testid={`${film.id}-action-callout`}><span>WHAT AHI DOES / {String(active + 1).padStart(2, '0')}</span><h3>{current.verb}</h3><p>{current.action}</p><i /></motion.article></AnimatePresence>
    <AnimatePresence mode="wait"><motion.aside key={`${active}-human`} className="laptop-human-callout" initial={reduced ? false : { opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={reduced ? undefined : { opacity: 0 }} transition={{ duration: reduced ? 0 : .35, delay: reduced ? 0 : .08 }} data-testid={`${film.id}-human-checkpoint`}><span><i /> HUMAN LINE</span><p>{current.human}</p></motion.aside></AnimatePresence>
